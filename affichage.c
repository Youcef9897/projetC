...
static void afficherSimulationComplete(Graphe *g) {
    ...
    DrawText("Inventaire -> poursuite + cameras -> detection -> intervention -> capture",
             40, 80, 20, DARKGRAY);
    ...
    if (etatSim == SIM_INVENTAIRE) {
    DrawText("MISSION 3 : preparation du materiel au depart.",
             40, 120, 22, DARKPURPLE);

    DrawText("Inventaire optimal charge avant la poursuite.",
             40, 155, 20, DARKGRAY);
    ...
    DrawText("CAMERA ACTIVE : suspect repere sur une zone deja surveillee.",
             40, 120, 22, RED);
    ...
    DrawText(TextFormat("Etape : %d", etapeChemin), x, y + 135, 18, BLACK);

    if (etatSim == SIM_INVENTAIRE) {
        DrawText("Etat : inventaire au depart", x, y + 180, 18, DARKPURPLE);
    } else if (etatSim == SIM_PARCOURS) {
        DrawText("Etat : poursuite + cameras", x, y + 180, 18, BLACK);
    } else if (etatSim == SIM_REPERE) {
        DrawText("Etat : suspect repere", x, y + 180, 18, RED);
    } else if (etatSim == SIM_CAPTURE) {
